...
var repPro = Replication.prototype;
repPro.cancel = function () {
  this.cancelled = true;
  this.emit('cancel');
};
repPro.CANCELLED = {};
repPro.isCanceled = function () {
  if (this.cancelled) {
    throw this.CANCELLED;
  }
};
repPro.startIter = function () {
  var self = this;
...
repPro.start = function (repId, src, target) {
  this.repId = repId;
  this.src = src;
  this.target = target;
  var self = this;
  function onChange() {
    ...
    .then(null, function (err) {
      if (err === self.CANCELLED) {
        // stuff got cancelled partway through, ignore it;
      } else {
        throw err;
      }
    });
...
repPro.updateCheckpoint = function (doc, direction) {
  self.isCanceled();
  doc = utils.clone(doc);
...
repPro.writeCheckpoints = function () {
  this.isCanceled();
  var doc = {
    last_seq: this.since,
    _id: this.repId
...
repPro.fetchCheckpoint = function () {
  var self = this;
  self.isCanceled();
...
repPro.revsDiff = function (changes) {
  this.isCanceled();
  var diff = {};
...
repPro.fetchRevs = function (diffs) {
  this.isCanceled();
...
repPro.fetchSingleRev = function (diff) {
  this.isCanceled();
...
repPro.writeChanges = function (docs) {
  this.isCanceled();
    ...
    return toPouch(target).then(function (target) {
      if (opts.server) {
        ...
        genReplicationId(src, target, opts, function (repId) {
          replicateRet.start(repId, src, target);
        });
      }
    });
  }).then(null, function (err) {
