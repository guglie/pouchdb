...
// global store of changeEmitter objects (one per db name)
...
function createDocMap(db) {
  return mapReduce(db, 'docCount', function (key, value, emit) {
    var ks = key.split('/');
    var outKey;
    if (ks.length > 1 && ks[0][0] === '_') {
      outKey = ks;
    } else {
      outKey = ['all', key];
    }
...
function LevelPouch(opts, callback) {
  ...
  var db, mapDB;
  var name = opts.name;
  var changeEmitter = changeEmitters[name] || new EventEmitter();
  if (typeof opts.createIfMissing === 'undefined') {
    opts.createIfMissing = true;
  }
  changeEmitters[name] = changeEmitter;
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function complete() {
      ...
      results.forEach(function (result) {
        ...
        changeEmitter.emit('change', change);
      });
      ...
      changeStream
        ...
        .on('close', function () {
          ...
          if (opts.continuous && !opts.cancelled) {
            changeEmitter.on('change', changeListener);
          }
    ...
    if (opts.continuous) {
      return {
        cancel: function () {
          ...
          opts.cancelled = true;
          if (changeListener) {
            changeEmitter.removeListener('change', changeListener);
          }
        }
      };
    }
